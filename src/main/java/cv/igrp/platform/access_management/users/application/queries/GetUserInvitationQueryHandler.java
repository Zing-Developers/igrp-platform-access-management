...
@Component
public class GetUserInvitationQueryHandler implements QueryHandler<GetUserInvitationQuery, ResponseEntity<InvitationDTO>> {

    private static final Logger LOGGER = LoggerFactory.getLogger(GetUserInvitationQueryHandler.class);
    ...
    public GetUserInvitationQueryHandler(InvitationEntityRepository invitationEntityRepository, InvitationMapper invitationMapper, UserUtils userUtils) {
    ...
    @IgrpQueryHandler
    public ResponseEntity<InvitationDTO> handle(GetUserInvitationQuery query) {
        final var invitationEntity = invitationEntityRepository.findByIdOrThrow(query.getId());
